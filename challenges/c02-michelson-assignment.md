...
**Observations**: - Write your observations here! - Distinctness 3 had a
mean velocity a bit under 2 km/s higher than Michelson’s value, while
Distinctness 1 and 2 were each calculated a bit under 2 km/s lower. -
Why might your table differ from Michelson’s? - The difference may be
due to Michelson rounding his reported values, as well as differences in
calculation precision compared to our modern machinery.
...
error <- abs(LIGHTSPEED_MICHELSON - LIGHTSPEED_VACUUM)
error
```
...
uncertainty) greater or less than the true error? - His uncertainty was
way less than the true error - Make a quantitative comparison between
Michelson’s uncertainty and his error. - Michelson’s uncertainty was 51
km/s, while his error was ~152 km/s, so his error was roughly triple the
value of his uncertainty.
...
**Observations**: Similarities - For both sets of data, most of the
means for each day fall within Michelson’s estimated uncertainty bounds,
but some go outside of it. - Differences - In Michelson’s real data,
there is a lot more variation in the daily means near the beginning of
the experiment, and then somewhat less later on. Meanwhile, the
simulated measurements are more evenly random over time and only go
outside of the uncertainty bounds near the end.
...
- Velocity vs Temp – Measured velocities seemed to go up a bit as
  temperature increased, although the relationship is pretty weak. There
  also seems to be a cluster of measurements between 60-70°F that are
  closer to the modern value.
- Velocity vs Distinctness – Measurements with higher distinctiveness
  appear to be a bit more tightly grouped, while lower-distinctness
  measurements showed more variability and a few more far-out values
  (unsure if it’s far enough to be outlier).
- Velocity vs Date – The measurements appear a lot more variable at
  first and tend to get more tightly grouped over time. This could
  potentially reflect some improvement in technique over the course of
  the experiment, but the plot alone can’t show what really caused the
  change.
